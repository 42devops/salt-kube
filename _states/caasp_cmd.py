...
def run(
    ...
    **kwargs
    ...
    retry_ = {"attempts": 1, "interval": 1, "until": None}
    retry_.update(retry)

    ret = None

    for attempt in range(retry_["attempts"]):
        ret = __states__["cmd.run"](
            name=name,
            onlyif=onlyif,
            unless=unless,
            creates=creates,
        ...
        if ret["result"]:
            ret_success = {
                ...
                "changes": ret["changes"],
                "result": True,
                "comment": "Command executed succesfully after {0} retries. Last output: {1}".format(
                    attempt + 1, ret["comment"]
                ),
            }
            ...
            retry_until_ret = __states__["cmd.run"](
                name=retry_["until"],
                cwd=cwd,
                runas=runas,
                shell=shell,
            ...
            ret["comment"] = (
                ret["comment"] + "(until: " + retry_until_ret["comment"] + ")"
            )
    ...
    return {
        ...
        "changes": ret["changes"],
        "result": False,
        "comment": "Command failed after {0} retries. Last output: {1}".format(
            retry_["attempts"], ret["comment"]
        ),
    }
