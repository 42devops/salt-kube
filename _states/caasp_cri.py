...
def wait_for_container(name, namespace, timeout=60, **kwargs):
    ...
    running = __salt__["caasp_cri.wait_for_container"](
        name, namespace, timeout, **kwargs
    )
    ...
    if running:
        ...
        ret[
            "comment"
        ] = "{namespace}.{container} was not restarted by kubelet within the given time".format(
            namespace=namespace, container=name
        )
