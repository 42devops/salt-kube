...
def running_stable(
    ...
    **kwargs
    ...
    __states__["service.running"](
        name=name, enable=enable, sig=sig, init_delay=init_delay
    )
    ...
    for retry in range(max_retries):
        pid = __salt__["service.status"](name=name, sig=sig)
        ...
        if current_successful_retries_in_a_row == successful_retries_in_a_row:
            ...
            ret[
                "comment"
            ] = "Service {0} is up after {1} total retries. Including {2} retries in a row".format(
    ...
    if not ret["result"]:
        ret[
            "comment"
        ] = "Service {0} is dead after {1} total retries. Expected {2} success in a row, got {3} as maximum".format(
            name,
            max_retries,
            successful_retries_in_a_row,
