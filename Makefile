...
init: all
	...
	sleep 10
	vagrant ssh salt -c "sudo salt-call state.sls packages"
	vagrant ssh minion01 -c "sudo salt-call state.sls packages"
	vagrant ssh minion02 -c "sudo salt-call state.sls packages"
	sleep 20
