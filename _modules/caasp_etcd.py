...
def get_cluster_size(**kwargs):
    ...
    if not member_count:
        ...
        if member_count < MIN_RECOMMENDED_MEMBER_COUNT:
            ...
            warn(
                "etcd member count too low (%d), increasing to %d",
                num_masters,
        ...
        if member_count < MIN_RECOMMENDED_MEMBER_COUNT:
            warn(
                "etcd member count too low (%d), consider increasing " "to %d",
                member_count,
                MIN_RECOMMENDED_MEMBER_COUNT,
            )
...
def get_additional_etcd_members(num_wanted=None, **kwargs):
    ...
    debug(
        "get_additional_etcd_members: curr:%d wanted:%d -> %d missing",
        num_current_etcd_members,
        num_wanted_etcd_members,
...
def get_endpoints(
    with_id=False, skip_this=False, skip_removed=False, port=ETCD_CLIENT_PORT, sep=","
):
    """
