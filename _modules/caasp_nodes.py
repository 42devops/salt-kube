...
def _get_prio_etcd(unassigned=False):
    ...
    res.append(
        "G@roles:etcd and not P@roles:(kube-master|kube-minion) and not G@bootstrap_complete:true"
    )
    ...
    if unassigned:
        ...
        res.append(
            "not P@roles:(kube-master|kube-minion|etcd) and not G@bootstrap_complete:true"
        )
        res.append("not P@roles:(kube-master|kube-minion|etcd)")

    # only-master nodes (preferring non-bootstrapped ones)
    res.append(
        "G@roles:kube-master and not G@roles:etcd and not G@bootstrap_complete:true"
    )
    res.append("G@roles:kube-master and not G@roles:etcd")

    # kubernetes minions (preferring non-bootstrapped ones)
    res.append(
        "G@roles:kube-minion and not G@roles:etcd and not G@bootstrap_complete:true"
    )
...
def is_admin_node():
    ...
    return any(role in ("salt", "ca") for role in node_roles)
